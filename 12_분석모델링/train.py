...
작업일자 : 2026-08-03
작업자 : 김기완
목적 : 통신 고객 이탈 분류 모델을 학습하고, 결과와 모델을 저장한다.
데이터 파일 : 12_분석모델링\ml_data\telecom_churn.csv

train.py 실행 --> streamlit 파일 실행

"""
# 라이브러리 불러오기
from pathlib import Path # 경로 실행
import json
...
# resolve() : 상대경로를 절대경로로 바꿔준다. (어디서 실행하든지 안전)
# parent : 파일이 들어있는 폴더
HERE = Path(__file__).resolve().parent
DATA_PATH = HERE /'ml_data'/'telecom_churn.csv'
# print(HERE.parents[0]) # C:\Users\Administrator\bigdata2026\bigdata2026-basic
# print(HERE.parents[1]) # C:\Users\Administrator\bigdata2026
# print(HERE.parents[2]) # C:\Users\Administrator

# 학습된 모델과 평가지표를 이 파일이 있는 폴더(HERE) 바로 아래에 저장
MODEL_PATH = HERE / 'churn_model.joblib'
METRICS_PATH = HERE / 'metrics.json'

# 모델 입력으로 사용할 열(특성, 피처) 목록을 명시적으로 나열
...
def build_pipeline() -> Pipeline:
    ...
    numeric_pipe = Pipeline([
        ...
        # 표준화로 스케일링 : monthly_fee(수만원 단위), complaints(0~5정도의 단위)처럼 단위 차이가 클 때 사용
...
def train() -> dict:
    ...
    metrics = {
        ...
        # recall_churn : 실제 이탈 고객 중 몇 %를 잡아냈냐? -> 이탈 예측에서는 이 값이 아주 중요(놓치면 안되는 고객이므로)
    ...
    # 평가 지표를 사람이 읽을 수 있도로 JSON 파일로 저장
    # ensure_ascii=False : 한글이 유니코드 secape(\uXXXX)로 깨지지 않고, 그대로 저장
