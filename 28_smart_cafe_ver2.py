...
def payment():
    ...
    while True:
        ...
        for i in menu:
            if i[0] == order:
                result.append([i[0], i[1], count])
    for i in result: # ['에스프레소',2000, 3]
        total += i[1] * i[2]
    return total
...
while True:
    ...
    if choice < 0 or choice > 3:
        ...
        category = input('종류별 메뉴 입력(예: 커피, 음료, 디저트 중 하나) ')
        for i in menu:
            if i[2] == category:
                result.append(i[:2])
    elif choice == 3:
        total = payment()  # 함수 호출
        print(f'주문 내역 확인 : {result}')
        print(f'지출할 총금액 : {total}원\n')
        continue
